...
# ================================================================
# Function: Add-GroupAsExclusionToAllApps with Stop support
# ================================================================
function Add-GroupAsExclusionToAllApps {
    ...
    $LogViewer.AppendText("🚀 Adding '$($groupObj.DisplayName)' as exclusion to all apps...`r`n")
    ...
    foreach ($app in $apps) {
        ...
        if ($alreadyExcluded) {
            $LogViewer.AppendText("✅ '$($groupObj.DisplayName)' already excluded on '$($app.DisplayName)'.`r`n")
        } else {
            try {
                New-MgDeviceAppManagementMobileAppAssignment -MobileAppId $app.Id -BodyParameter @{
                    target = @{
                        "@odata.type" = "#microsoft.graph.exclusionGroupAssignmentTarget"
                        groupId = $groupObj.Id
                    }
                ...
                $LogViewer.AppendText("🚫 Added exclusion for '$($groupObj.DisplayName)' on '$($app.DisplayName)'.`r`n")
            } catch {
                $LogViewer.AppendText("❌ Failed to add exclusion for '$($groupObj.DisplayName)' on '$($app.DisplayName)'.`r`n")
            }
        }
    }
    $LogViewer.AppendText("✅ Done adding exclusions across all apps.`r`n")
}

# ================================================================
# GUI Form and Controls
# ================================================================
$Form = New-Object system.Windows.Forms.Form
$Form.ClientSize = New-Object System.Drawing.Point(950, 750)
$Form.Text = "Intune App Assignment Tool with Global Exclusion"
$Form.TopMost = $false
...
$LogViewer.Size = New-Object System.Drawing.Size(880, 350)
...
$ShowSummaryCheckbox.Location = New-Object System.Drawing.Point(20, 600)
...
$Submit.Width = 120
$Submit.Height = 40
$Submit.Location = New-Object System.Drawing.Point(780, 600)
...
$Export.Width = 160
$Export.Height = 40
$Export.Location = New-Object System.Drawing.Point(600, 600)
...
$Stop.Width = 120
$Stop.Height = 40
$Stop.Location = New-Object System.Drawing.Point(420, 600)
$Form.Controls.Add($Stop)

$GlobalExclusion = New-Object System.Windows.Forms.Button
$GlobalExclusion.Text = "Add Exclusion To All Apps"
$GlobalExclusion.Width = 240
$GlobalExclusion.Height = 40
$GlobalExclusion.Location = New-Object System.Drawing.Point(20, 650)
...
# ================================================================
# Global Exclusion Click (with Stop support)
# ================================================================
$GlobalExclusion.Add_Click({
    $global:StopAudit = $false
    $groupName = $GroupInputExc.Text.Split(",")[0].Trim()
    if (-not $groupName) {
        [System.Windows.Forms.MessageBox]::Show("Please enter at least one group name in the Excluded box.")
        return
    }
    Add-GroupAsExclusionToAllApps -GroupName $groupName -LogViewer $LogViewer
})
